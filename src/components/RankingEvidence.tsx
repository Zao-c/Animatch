...
interface EvidenceItem {
  animeId: string; title: string; imageUrl: string | null; participantCount: number; comparisonCount?: number;
...
export function RankingEvidencePanel({ items, season = false }: { items: EvidenceItem[]; season?: boolean }) {
  ...
  return <section className="my-4 rounded-xl border border-cyan-300/15 bg-cyan-300/[0.035] p-3 sm:p-4" aria-label="榜单样本说明">
    ...
    <details className="mt-1 text-xs text-slate-400"><summary className="min-h-11 cursor-pointer py-3">如何理解这些数字</summary><p className="leading-6">覆盖率是评价这部作品的人数占本榜有效参与者的比例。离散度越高，个人 Elo 的差异越大，不能直接理解为作品质量差。正式榜沿用现有计分；小众发现展示 3–9 位评价者、至少 6 次比较、平均 Elo 高于初始值的作品，按平均 Elo 排列，不代表已形成社区共识。</p></details>
